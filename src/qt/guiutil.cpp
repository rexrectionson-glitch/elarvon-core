...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter an ELARVON address (e.g. %1)").arg(
...
{
    // Return if URI is invalid or does not use the ELARVON payment scheme.
    if(!uri.isValid() || uri.scheme() != QString("elarvon"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("elarvon:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ELARVON.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ELARVON (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("ELARVON (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // Check for ELARVON*.lnk.
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == ChainType::MAIN)
            optionFile << "Name=ELARVON\n";
        else
            optionFile << strprintf("Name=ELARVON (%s)\n", ChainTypeToString(chain));
