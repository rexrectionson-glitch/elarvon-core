    ...
    m_network_style(networkStyle)
{
    ...
    {
        progressBar->setStyleSheet("QProgressBar { color: white; background-color: #DED6CA; border: 0; border-radius: 7px; padding: 1px; text-align: center; } QProgressBar::chunk { background: #B58A45; border-radius: 7px; margin: 0px; }");
    }
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to an ELARVON address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and elarvon: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your ELARVON addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified ELARVON addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed ELARVON Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed ELARVON Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open an elarvon: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible ELARVON command-line options").arg(CLIENT_NAME));
...
void BitcoinGUI::createMenuBar()
{
    ...
    QAction* explorer_action = help->addAction(tr("ELARVON &Explorer"));
    explorer_action->setStatusTip(tr("Open the ELARVON blockchain explorer"));
    connect(explorer_action, &QAction::triggered, [] {
        QDesktopServices::openUrl(QUrl(QStringLiteral("https://elarvonexplorer.com")));
    });
    help->addSeparator();
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to the ELARVON network.", "", count);
    } else {
        //: A substring of the tooltip.
