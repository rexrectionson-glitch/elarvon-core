...
const QString BITCOIN_IPC_PREFIX("elarvon:");
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start elarvon: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("elarvon://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'elarvon://' is not a valid URI. Use 'elarvon:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid ELARVON address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
