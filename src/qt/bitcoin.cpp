...
static const char* qt_argv = "elarvon-qt";
...
void BitcoinApplication::parameterSetup()
{
    ...
    // Desktop wallets synchronize as outbound clients by default. This avoids
    // local port reservations and immediately connects a fresh installation to
    // the public ELARVON bootstrap node. Explicit user settings still win.
    gArgs.SoftSetBoolArg("-listen", false);
    if (!gArgs.IsArgSet("-connect")) {
        gArgs.SoftSetArg("-addnode", "2.57.90.77:19733");
    }

    // The desktop wallet is for holding and transacting. Block production
    // remains available to dedicated bitcoind nodes such as the public VPS.
    gArgs.ForceSetArg("-disableminingrpc", "1");
...
int GuiMain(int argc, char* argv[])
{
    ...
    app.setStyleSheet(ElarvonStyleSheet());
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see elarvon-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                QString::fromStdString("Command line contains unexpected token '%1', see elarvon-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
