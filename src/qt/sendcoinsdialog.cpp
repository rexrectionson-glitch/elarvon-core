...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        if (model->wallet().hasExternalSigner()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed ELARVON Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
        }
...
bool SendCoinsDialog::PrepareSendText(QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    if (model->wallet().privateKeysDisabled() && !model->wallet().hasExternalSigner()) {
        ...
        question_string.append(tr("Please, review your transaction proposal. This will produce a Partially Signed ELARVON Transaction (PSBT) which you can save or copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
        ...
        question_string.append(tr("Please, review your transaction. You can create and send this transaction or create a Partially Signed ELARVON Transaction (PSBT), which you can save or copy and then sign with, e.g., an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
    } else {
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid ELARVON address"));
        }
        else // Valid address
        {
