...
{
    static const CRPCCommand information_commands[]{
        {"mining", &getnetworkhashps},
        {"mining", &getmininginfo},
    };
    for (const auto& c : information_commands) {
        t.appendCommand(c.name, &c);
    }

    // ELARVON Core's GUI sets this internal flag. Dedicated bitcoind nodes do
    // not, so production nodes retain the RPCs needed to build the chain.
    if (gArgs.GetBoolArg("-disableminingrpc", false)) return;

    static const CRPCCommand production_commands[]{
    ...
    for (const auto& c : production_commands) {
        t.appendCommand(c.name, &c);
    }
}
