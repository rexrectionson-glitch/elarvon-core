...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see elarvond -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(interfaces::Init& init, ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The " CLIENT_NAME " daemon (elarvond) is a headless program that connects to the ELARVON network to validate and relay transactions and blocks, as well as relaying addresses.\n\n"
                "It provides the backbone of the ELARVON network and its RPC, REST and ZMQ services can provide various transaction, block and address-related services.\n\n"
